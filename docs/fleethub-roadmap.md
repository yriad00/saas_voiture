...
## CONTRACT CRITICAL-FIRST + E2E ASSERTION HARDENING — 18 septembre 2026

- `getContractCritical` is now a small, tenant-scoped read model for the first paint: contract/customer/vehicle/reservation, status/dates, preparation/check-out/check-in state, live balance and deposit summary. The existing full `getContract` dossier remains intact for secondary panels.
- `ContractSecondarySections` streams the existing operational panels behind a Suspense boundary: payments, invoices, photos, inspections, participants, extras, documents, signatures, swaps, extensions, owner settlement, return charges and audit-related history. Permissions and existing RLS-scoped reads are unchanged.
- Browser assertions no longer depend on the transient `Réservation créée` panel. One-way, double-booking, extension-conflict and category flows identify the persisted reservation by a unique synthetic marker, then assert URL and database state. Local release validation: **23/23 passed, 0 failed, 0 skipped**; standard rental: **1/1 passed**; one-way: **1/1 passed**.
- Critical summary cards use distinct anchors (`finances-summary`, `caution-summary`) so the streamed financial/caution sections retain their stable operational anchors and do not create duplicate DOM IDs.
- API mutation routes now validate same-origin requests by browser host (including forwarded host) rather than an internal reconstructed URL, preserving the CSRF boundary behind local/proxy runtimes. Auth, permission and RLS checks remain in the underlying actions.
- Remaining hosted HTTPS measurement is external: no `FLEETHUB_HOSTED_URL` is configured in this workspace, so release validation must stay fail-closed until a staging deployment URL is available.
