...
function Submit() { const { pending } = useFormStatus(); return <Button type="submit" disabled={pending}>{pending && <Loader2 className="animate-spin" />}Valider le check-out</Button>; }
export function CheckoutForm({ contractId, checkout }: { contractId: string; checkout?: Tables<"contract_checkouts"> | null }) {
  ...
  return <div className="space-y-4"><form action={action} className="space-y-4"><input type="text" className="hidden" name="contract_id" value={contractId}  readOnly />
    ...
    {state.error && <p className="text-sm text-destructive">{state.error}</p>}{state.success && <p className="text-sm text-emerald-600">Check-out enregistré. Ajoutez maintenant les photos de départ ci-dessous.</p>}<Submit />
