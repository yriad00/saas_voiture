...
function Submit() { const { pending } = useFormStatus(); return <Button type="submit" disabled={pending}>{pending && <Loader2 className="animate-spin" />}Enregistrer la préparation</Button>; }
...
export function PreparationForm({ contractId, preparation }: { contractId: string; preparation?: Tables<"vehicle_preparations"> | null }) {
  ...
  return <form action={action} className="space-y-4"><input type="text" className="hidden" name="contract_id" value={contractId}  readOnly />
    ...
    {state.error && <p className="text-sm text-destructive">{state.error}</p>}{state.success && <p className="text-sm text-emerald-600">Préparation enregistrée.</p>}<Submit />
  </form>;
}
