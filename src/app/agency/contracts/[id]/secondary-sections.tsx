...
export async function ContractSecondarySections({ id, agencyId, ctx }: SecondaryProps) {
  const c = await getContract(id, agencyId);
  if (!c) return null;
  const deposit = c.deposit;
  const supabase = await createClient();
  const [{ data: agency }, { data: agencySettings }, { data: branches }] = await measurePerf("contract.secondary.agencyConfig", () => Promise.all([
    supabase.from("agencies").select("name,address,city,phone,email,logo_url,currency").eq("id", agencyId).maybeSingle(),
    supabase.from("agency_settings").select("extra").eq("agency_id", agencyId).maybeSingle(),
    supabase.from("branches").select("id,name").eq("agency_id", agencyId).eq("active", true).order("name"),
  ]));
  const [{ data: extras }, { data: assignedExtras }] = await measurePerf("contract.secondary.extras", () => Promise.all([
    listActiveExtras(supabase, agencyId, c.branch_id),
    getContractExtras(supabase, id, agencyId),
  ]));
  const [{ data: rentalParticipants }, { data: participantCustomers }] = await measurePerf("contract.secondary.participants", () => Promise.all([
    (supabase as any).from("rental_participants").select("customer_id,role").eq("contract_id", id).eq("agency_id", agencyId),
    supabase.from("customers").select("id,first_name,last_name").eq("agency_id", agencyId).is("deleted_at", null).order("last_name").limit(200),
  ]));
  const ownerSettlementQuery = c.vehicle.ownership_type === "SUBLEASE"
    ? (supabase as any).from("vehicle_owner_settlements").select("id,owner_amount,paid_amount,status,notes").eq("contract_id", id).eq("agency_id", agencyId).maybeSingle()
    : Promise.resolve({ data: null });
  const [{ data: ownerSettlement }, { data: preparation }, { data: checkout }, { data: checkin }, { data: signatures }, { data: rentalUpdates }, { data: extensions }] = await measurePerf("contract.secondary.dossierPanels", () => Promise.all([
    ownerSettlementQuery,
    supabase.from("vehicle_preparations").select("*").eq("contract_id", id).eq("agency_id", agencyId).maybeSingle(),
    supabase.from("contract_checkouts").select("*").eq("contract_id", id).eq("agency_id", agencyId).maybeSingle(),
    (supabase as any).from("contract_checkins").select("*").eq("contract_id", id).eq("agency_id", agencyId).maybeSingle(),
    (supabase as any).from("contract_signatures").select("id,signer_type,signer_name,signature_data,signed_at,contract_version").eq("contract_id", id).eq("agency_id", agencyId).order("signed_at", { ascending: false }),
    supabase.from("active_rental_updates").select("id,event_type,occurred_at,mileage,fuel_level,location,notes").eq("contract_id", id).eq("agency_id", agencyId).order("occurred_at", { ascending: false }).limit(20),
    supabase.from("rental_extensions").select("id,previous_end_date,new_end_date,added_days,extra_amount,reason,created_at").eq("contract_id", id).eq("agency_id", agencyId).order("created_at", { ascending: false }),
  ]));
  let swapVehicleQuery = supabase.from("vehicles").select("id,brand,model,license_plate").eq("agency_id", agencyId).eq("status", "AVAILABLE").is("deleted_at", null).neq("id", c.vehicle.id).order("brand");
  swapVehicleQuery = c.branch_id ? swapVehicleQuery.eq("branch_id", c.branch_id) : swapVehicleQuery.is("branch_id", null);
  const [{ data: swapVehicles }, { data: swaps }] = await measurePerf("contract.secondary.swapHistory", () => Promise.all([
    swapVehicleQuery,
    supabase.from("vehicle_swaps").select("id,old_vehicle_id,new_vehicle_id,reason,created_at").eq("contract_id", id).eq("agency_id", agencyId).order("created_at", { ascending: false }),
  ]));
